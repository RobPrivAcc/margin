...
class Product extends PDOException{
    ...
    function allProdFromBrand($brandName,$isDiscontinued){
        ...
        $where = "";
            ...
            $sql = "SELECT [Name of Item], [Selling Price], ([Supplier Cost] - ([Supplier Cost] * ([SuppDis1]/100))) as 'Cost', [POR], [TaxRate],[PackSize],[BarCode], [SuppDis1] FROM [Stock] WHERE ".$where.$this->isDiscontinued." ORDER BY [Name of Item] ASC";
            ...
            while($row = $query->fetch()){
                if($row['TaxRate'] > 0){
                    $por = ($row['Selling Price']/(($row['TaxRate']/100)+1)-$row['Cost'])/($row['Selling Price']/(($row['TaxRate']/100)+1));
                }else {
                    $por = ($row['Selling Price']  - $row['Cost']) / $row['Selling Price'];
                }
                $prodFromSupArray[] = array('name' => $this->nameChange($row['Name of Item'],'set'),
                                            'salePrice' => round($row['Selling Price'],2),
                                            'ean'=>$row['BarCode'],
                                            'supCost' => round($row['Cost'],2),
                                            'POR' => $por,
                                            'Tax' => round($row['TaxRate'],2),
                                            'pack' => round($row['PackSize']),
                                            'discount' => $row['SuppDis1']
                    );
            }
